...
| AC-3          | The filter offers the permitted case types with the current one pre-selected                                                                    | _The case-type filter offers the available types_             | …→ "builds the case-type select with the current selection marked"                                                                                                    |
...
| AC-6 _(edge)_ | Opening with no choice asks for the default report (no case type named)                                                                         | _Opening the page with no choice asks for the default report_ | …→ "calls /cases/report with no query string when criteria is falsy"                                                                                                  |

**Error/edge coverage:** AC-4, AC-6 = 2 of 6 scenarios (33%). Below the 40% target in isolation;
the combined cross-repo suite for FGP-1221 (backend 40% + frontend 33%) sits at ~38%. Accepted
for a simple prototype; the cheapest top-up would be a frontend scenario for a backend/service
failure surfacing a friendly error (currently relies on the shared `errors` plugin — see review).
