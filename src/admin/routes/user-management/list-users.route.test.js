...
vi.mock("../../../auth/use-cases/find-admin-users.use-case.js");
...
describe("listUsersRoute", () => {
  ...
  it("renders users page", async () => {
    findAdminUsersUseCase.mockResolvedValue([
      {
        id: "user-2",
        name: "Zara Zee",
  ...
  it("passes auth context to findAdminUsersUseCase", async () => {
    findAdminUsersUseCase.mockResolvedValue([]);
    ...
    expect(findAdminUsersUseCase).toHaveBeenCalledWith(
      {
        token: "mock-token",
        user: { id: "user-123" },
      },
  ...
  it("returns 403 when backend forbids listing users", async () => {
    findAdminUsersUseCase.mockRejectedValue(Boom.forbidden("Forbidden"));
