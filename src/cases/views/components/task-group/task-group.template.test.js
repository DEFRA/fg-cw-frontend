...
describe.skip("Task Groups", () => {
  ...
  describe("Component", () => {
    ...
    test("Should render task links with correct text and href", () => {
      ...
      expect(taskLinks.eq(0).attr("href")).toBe("/cases/123/task/1");
      expect(taskLinks.eq(1).text().trim()).toBe("Task 2");
      expect(taskLinks.eq(1).attr("href")).toBe("/cases/123/task/2");
    });
    ...
    test("Should render stage action button", () => {
      ...
      expect(actionButton.attr("formaction")).toBe("/cases/123");
    });
  });
});
